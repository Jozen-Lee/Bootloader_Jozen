...
static void MY_Read_Flash(uint8_t* pBuffer,uint32_t ReadAddr,uint32_t NumByteToRead)
{
	...
	else
	{
		...
		for(temp = 0; temp < num; temp ++)
		{
			SPI_Flash_Read(pBuffer+temp*0xFFFF, ReadAddr+temp*0xFFFF, 0xFFFF);
		}
		SPI_Flash_Read(pBuffer+temp*0xFFFF, ReadAddr+temp*0xFFFF, NumByteToRead%0xFFFF);
	}
}
...
static void Read_Pro_Name_Flash(void)
{
	...
	for(uint8_t i = 0; i < PRO_MAX_NUM; i++)
	{
		...
		if((uint8_t)name[0] != 0xFF) 																							//说明存有数据
		{
			name_length = Pro_Name_Length(name);																		//获取程序名长度
			HAL_UART_Transmit(&huart1,(uint8_t*)name,name_length,MAX_WAIT_TIME);							//有程序则打印程序名
		}	
		else 
		{
...
static uint8_t Choose_Pro(uint8_t mode)
{
	...
	while(1)
	{
		if(USART_RX_CNT)
		{
			...
			{
				while(Progress_Buff[temp] && Progress_Buff[temp] != 0x0D) 
				{
	 	...
	 	if(USART_RX_CNT)
		{
			if(oldcount==USART_RX_CNT)	//新周期内,没有收到任何数据,认为本次数据接收完成.
			{
				if(USART_RX_CNT > 50)	//防止误操作
				{
					...
					MY_Write_Flash(Progress_Buff, add, PRO_MAX_SIZE);													//将程序存储进FLASH
